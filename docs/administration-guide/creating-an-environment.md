...
## Create a YAML properties file for your Cerberus environment

Here is an example Cerberus environment [YAML properties file](https://github.com/Nike-Inc/cerberus-lifecycle-cli/blob/feature/envinronmental_yaml_dsl/src/main/resources/example-standup.yaml).


## Create the VPC for the Cerberus Environment and set up the base resources needed for the environment
You will need to supply the following properties in your YAML:
...
Here is the command to run with our demo values, make sure to replace the values with your own.
    ...
    cerberus --debug -f /path/to/environment/properties.yaml create-base
...
For the demo environment I am going to whitelist my current public IP4 Address.
    ...
    cerberus --debug -f /path/to/environment/properties.yaml whitelist-cidr-for-vpc-access
...
Upload the Consul Certificate
    ...
    cerberus --debug \
    -f /path/to/environment/properties.yaml \
    upload-cert \
    --stack-name consul
...
Create the config needed for consul to run
    ...
    cerberus --debug -f /path/to/environment/properties.yaml create-consul-config
...
Create an ACL for Vault to have perms to use Consul
    ...
    cerberus --debug -f /path/to/environment/properties.yaml create-vault-acl


## Create Consul Cluster

Create the Consul Cluster, for this command we will need the following properties in your YAML:
...
costcenter    | Provide a cost center name or other value (`no-op` would work).  This value will be tagged on all resources possible and is helpful for internal billing or other purposes. (This is specific to how Nike tags resources and should be made optional in the future).
    ...
    cerberus --debug -f /path/to/environment/properties.yaml create-consul-cluster
...
Create the vault configuration that will enable TLS and have the consul config
    ...
    cerberus --debug -f /path/to/environment/properties.yaml create-vault-config
...
costcenter    | Provide a cost center name or other value (`no-op` would work).  This value will be tagged on all resources possible and is helpful for internal billing or other purposes. (This is specific to how Nike tags resources and should be made optional in the future).
    ...
    cerberus --debug -f /path/to/environment/properties.yaml create-vault-cluster
...
proxy you can do so by adding a YAML block to your Cerberus environment properties YAML file:
...
** **Note: You may need to wait a few minutes, after creating the Vault cluster, before you run this command.**
    ...
    cerberus --debug -f /path/to/environment/properties.yaml init-vault-cluster
...
then, run this command
    ...
    cerberus --debug -f /path/to/environment/properties.yaml unseal-vault-cluster
...
again this command talks directly to the instances so include proxy information if needed:
    ...
    cerberus --debug -f /path/to/environment/properties.yaml vault-health
...
if needed.
    ...
    cerberus --debug -f /path/to/environment/properties.yaml load-default-policies
...
when they authenticate through CMS. This command needs proxy information if available.
    ...
    cerberus --debug -f /path/to/environment/properties.yaml create-cms-vault-token
...
You can add arbitrary properties that will get written to the CMS configuration file by adding a
...
For the demo environment I will run the command with the following:
    ...
    cerberus --debug -f /path/to/environment/properties.yaml create-cms-config
...
costcenter    | Provide a cost center name or other value (`no-op` would work).  This value will be tagged on all resources possible and is helpful for internal billing or other purposes. (This is specific to how Nike tags resources and should be made optional in the future).
    ...
    cerberus --debug -f /path/to/environment/properties.yaml create-cms-cluster
...
Now we need to upload the Single Page App to S3
    ...
    cerberus --debug -f /path/to/environment/properties.yaml publish-dashboard
...
Create the reverse proxy config for NGINX
    ...
    cerberus --debug -f /path/to/environment/properties.yaml create-gateway-config
...
costcenter     | Provide a cost center name or other value (`no-op` would work).  This value will be tagged on all resources possible and is helpful for internal billing or other purposes. (This is specific to how Nike tags resources and should be made optional in the future).
    ...
    cerberus --debug -f /path/to/environment/properties.yaml create-gateway-cluster
...
If you would like to enable Slack integration and have the lambda message a slack channel when IPs are added or removed from the auto list you can configure the follwing properties in your YAML:
